...
# ---------------------------------------------------------------- 1. ps1
# NOTE (BrowserSkill repo): ./scripts/ holds this repo's own CI test fixtures.
# install-windows.test.ps1 deliberately contains Chinese text (it tests that
# Unicode install paths survive) plus a UTF-8 BOM; those fixtures never run on
# the user-side watcher, so they are exempt from the ASCII scan below.
while IFS= read -r -d '' f; do
    if LC_ALL=C grep -qn '[^[:print:][:space:]]' "$f"; then
        echo "[FAIL] non-ASCII bytes in $f  (keep .ps1 ASCII-only; put Chinese in .md/.json)"
        fail=1
    fi
done < <(find . -name '*.ps1' -not -path './.git/*' -not -path './scripts/*' -print0)
if [ "$fail" -eq 0 ]; then
