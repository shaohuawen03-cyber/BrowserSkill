...
# ---------------------------------------------------------------- scope
# This repo (BrowserSkill) ships PowerShell files that are NOT part of the
# git-sync skill and must not be judged by it:
#   ./install.ps1                       the bsk CLI installer (README: irm ... | iex)
#   ./scripts/install-windows.test.ps1  Pester tests for that installer
# Both contain UTF-8 em dashes on purpose, and ./install.ps1 is deliberately
# NOT a mirror of skills/git-sync/scripts/install.ps1 (that copy stays inside
# the skill so the repo's own installer survives an install/upgrade). The
# ASCII / drift / parse checks below are therefore scoped to git-sync files.
is_gitsync_ps1() {  # $1 = path -> 0 when the git-sync skill owns that .ps1
    case "$1" in
        ./install.ps1|./scripts/*) return 1 ;;
        *) return 0 ;;
    esac
}
...
if [ "$fail" -eq 0 ]; then
    echo "OK: all git-sync .ps1 files are ASCII-only (repo-owned ./install.ps1 and ./scripts/ skipped)"
fi
...
# NOTE: 'install' is intentionally absent from this list. This repo's root
# install.ps1 is BrowserSkill's own bsk CLI installer; the git-sync copy lives
# at skills/git-sync/scripts/install.ps1 and is never mirrored to the root.
drift=0
...
Get-ChildItem -Path . -Recurse -Filter *.ps1 | ForEach-Object {
    ...
    if ($_.Name -eq "install.ps1" -and $_.DirectoryName -eq $root) { return }
    if ($_.FullName -match "[\\/]scripts[\\/]install-windows\.test\.ps1$") { return }
    $t = $null
    $e = $null
...
if ($bad -eq 0) { Write-Output "OK: every git-sync .ps1 parses" }
exit $bad'
