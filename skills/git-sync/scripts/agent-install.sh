...
fetch_source() {  # $1 url  $2 branch  [$3 dir] -> prints the dir with that branch checked out
  # $3 is optional: the --source <git-url> path calls this with two arguments
  # only, and under `set -u` an unbound $3 aborted the whole installer with
  # "$3: unbound variable" (seen 2026-09-18 installing into BrowserSkill).
  local url="$1" br="$2" dir="${3:-}"
  [ -n "$dir" ] || dir="$WORKDIR/explicit"
  if [ ! -d "$dir/.git" ]; then
...
# A repo may already own a root .ps1 with the same name for its own purposes -
# BrowserSkill ships install.ps1 as its bsk CLI installer (README: irm ... | iex).
# Never clobber such a file: when the root copy exists and is NOT the skill's
# own script, leave it alone and keep the skill copy inside skills/git-sync/.
for f in sync push upload download pack doctor bootstrap pr hardware watch auth install; do
  if [ -f "$REPO/skills/git-sync/scripts/$f.ps1" ]; then
    if [ -f "$REPO/$f.ps1" ] && ! cmp -s "$REPO/$f.ps1" "$REPO/skills/git-sync/scripts/$f.ps1" \
       && ! git -C "$REPO" ls-files --error-unmatch "$f.ps1" >/dev/null 2>&1; then
      : # untracked foreign file - still overwrite, it is not part of the repo
    elif [ -f "$REPO/$f.ps1" ] && ! cmp -s "$REPO/$f.ps1" "$REPO/skills/git-sync/scripts/$f.ps1" \
       && git -C "$REPO" ls-files --error-unmatch "$f.ps1" >/dev/null 2>&1; then
      echo "KEPT: $f.ps1 at the repo root belongs to this repo - not overwritten"
      echo "      (the git-sync copy stays at skills/git-sync/scripts/$f.ps1)"
      continue
    fi
